...
// Usser Registerattion  route
//router.post('/register)',async(req,res) => {try{}catch{}})
// 
router.post("/register", async (req, res) => { 
  try {
    ...
    if (employeeExists) {
      return res
        .status(200)
        .send({ message: "employee already exists", success: false });
    }
    ...
    res
      .status(200)
      .send({ message: "employee created successfully", success: true });
  } catch (error) {
    // console.log(error);
    res 
      .status(500)
      .send({ message: "Error creating employee", success: false, error });
  }
});
...
router.post("/apply-doctor-account", authMiddleware, async (req, res) => {
  try {
    const newdoctor = new Doctor({ ...req.body, status: "pending admin approval" });
    // save applicant
...
router.post("/create-employee", async (req, res) => {
  try {
    ...
    if (employeeExists) {
      return res
        .status(200)
        .send({ message: "Employee already exists in DB/employeeRoute Error", success: false });
    }
    const password = req.body.password;
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);
